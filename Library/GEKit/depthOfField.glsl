...
uniform float manualFocusDepth;  //focal distance value in meters, but you may use autofocus option below
uniform float fstop; //f-stop value
uniform bool showFocus; //show debug focus point and focal range (red = focal point, green = focal range)


/* 
make sure that these values are the same for your camera, otherwise distances will be wrong.
*/

float znear = 0.1;			//camera clipping start
float zfar = 100.0;			//camera clipping end
float focalLength = 50.0; 	//camera focal length
...
float namount = 0.0005; //dither amount
...
vec2 rand(vec2 coord) //generating noise/pattern texture for dithering
{
	...
	float noiseX = clamp(fract(sin(dot(coord ,vec2(12.9898,78.233*timer))) * 43758.5453),0.0,1.0)*2.0-1.0;
	float noiseY = clamp(fract(sin(dot(coord ,vec2(12.9898,78.233)*2.0*timer)) * 43758.5453),0.0,1.0)*2.0-1.0;
